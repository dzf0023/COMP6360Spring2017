...
void* receiverThread(void* args)
{
    ...
    myaddr.sin_port = htons(oxygen_display_port);
...
void* heartDisplayThread(void* args)
{
    for (int i = 0;; i++)
    {
        cout<<heartRateString<<endl;
...
int main()
{
    ...
    pthread_create(&tids[1], NULL, heartDisplayThread,NULL);
    pthread_exit(NULL);
